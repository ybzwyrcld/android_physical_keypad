...
import com.android.launcher2.HomeEditView.OnClickPageListener;
import com.android.launcher2.HomeEditView.PageInfo;
...
public final class Launcher extends Activity
    ...
    private static final int MENU_SEARCH = MENU_EDIT + 1;
    private static final int MENU_LAUNCHER_SETTINGS = MENU_SEARCH + 1;
    ...
    private enum State { NONE, WORKSPACE, APPS_CUSTOMIZE, APPS_CUSTOMIZE_SPRING_LOADED, EDIT_PAGES };
    ...
    static int getScreenCount(Context context) {
        ...
        return pref.getInt("pages_count", SCREEN_COUNT);
    }
    ...
    protected void onStop() {
        super.onStop();
        if (mState == State.EDIT_PAGES) exitEditState();
    ...
    protected void onResume() {
        ...
        if (mState == State.WORKSPACE) {
            getWorkspace().resetChildRotation();
        }
...
*/
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        Log.d(TAG, "Launcher onCreateOptionsMenu");
        ...
        menu.add(0,MENU_EDIT, 0, R.string.menu_edit_pages)
        ...
        menu.add(0, MENU_LAUNCHER_SETTINGS, 0, R.string.launcher_setting)
        .setIcon(android.R.drawable.ic_menu_preferences)
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        Log.d(TAG, "Launcher onPrepareOptionsMenu");
        ...
        menu.findItem(MENU_EDIT).setVisible(!allAppsVisible);
    ...
    private void showLongPressMenu() {
        String[] items = {
                getString(R.string.wallpaper_instructions),
                getString(R.string.menu_edit_pages),
        };
        ...
        mDialog = new AlertDialog.Builder(this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT)
                ...
                .setItems(items, itemlistener ).create();
        mDialog.show();
    }

    private void showSetWallpaperMenu() {
        String[] items = {
                getString(R.string.home),
                getString(R.string.menu_lockscreen),
                getString(R.string.menu_lockscreen_and_home)
        };
        OnClickListener itemlistener = new OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                startWallpaper(which);
                dialog.dismiss();
            }
        };
        mDialog = new AlertDialog.Builder(this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT)
    ...
    private void startWallpaper(int mode) {
        showWorkspace(true);
        String action = null;
        switch (mode) {
        case 0:
        ...
        case 1:
            action = "com.leadcore.action.SET_LOCKSCREEN_WALLPAPER";
            break;
        case 2:
            action = "com.leadcore.action.SET_LOCKSCREEN_AND_HOME_WALLPAPER";
        default:
            break;
        }
        final Intent pickWallpaper = new Intent(action);
...
//        }
    ...
    private final ArrayList<ApplicationInfo> mUpdateAppList = new ArrayList<ApplicationInfo>();
    /**
    ...
    public void bindAppsUpdated(final ArrayList<ApplicationInfo> apps) {
        if (mWorkspaceLoading) return;
        synchronized (mUpdateAppList) {
            if (mUpdateAppList.isEmpty()) {
                mUpdateAppList.addAll(apps);
            } else {
                ArrayList<ApplicationInfo> tempAppList = new ArrayList<ApplicationInfo>();
                for (ApplicationInfo app : apps) {
                    tempAppList.add(app);
                    for (ApplicationInfo app1 : mUpdateAppList) {
                        if (app.componentName.equals(app1.componentName)) {
                            tempAppList.remove(app);
                            break;
                        }
                    }
                }
                if (!tempAppList.isEmpty()) {
                    mUpdateAppList.addAll(tempAppList);
                }
            }
            if (waitUntilResume(new Runnable() {
                    public void run() {
                        bindAppsUpdated(mUpdateAppList);
                    }
                }, true)) {
                return;
            }
    ...
    private HomeEditView mDragGrid;
    private void startEdit() {
        ...
        for (int i = 0; i < mWorkspace.getChildCount(); i++) {
            // create bitmap for homeeditview
            CellLayout view = (CellLayout) mWorkspace.getChildAt(i);
            HomeEditViewItem workspace_layout = HomeEditViewItem.newInstance(this);
            if (i == mWorkspace.getDefaultPage()) {
                workspace_layout.setDefault(true);
            }
            if (((ViewGroup) view.getChildAt(0)).getChildCount() != 0) {
                workspace_layout.setCellEmpty(false);
            } else {
                workspace_layout.setCellEmpty(true);
            }
            ImageView image = (ImageView) workspace_layout.findViewById(R.id.workspace_image);
            Bitmap bitmap = Bitmap.createBitmap(view.getWidth(), view.getHeight(), Bitmap.Config.ARGB_8888);
            // draw
            final Canvas c = new Canvas(bitmap);
            view.dispatchDraw(c);
            image.setImageBitmap(bitmap);
            views.add(workspace_layout);
        }

        mDragGrid.addViews(views);

        mDragGrid.setVisibility(View.VISIBLE);
        View edit_layout = findViewById(R.id.home_edit_layout);
        ...
        mState = State.EDIT_PAGES;
    }

    private OnClickPageListener listener = new OnClickPageListener() {

        @Override
        public void onClickPage(final int index, List<PageInfo> pages) {
    ...
    private void exitEditState() {
        mDragGrid.dismissDialog();
        List<PageInfo> pages = mDragGrid.getPagesInfo();
        mWorkspace.setVisibility(View.VISIBLE);
        ...
        if (mWorkspace.getCurrentPage() >= pages.size()) {
            mWorkspace.setCurrentPage(mWorkspace.getDefaultPage());
        }

        mState = State.WORKSPACE;
        mDragGrid.removeAllViews();
        System.gc();
    }
